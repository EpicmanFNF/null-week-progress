...
function onLoad(){
    ...
    bgSky2 = new BGSprite('stages/dead-pixel/weebSky', 0, 0, 0.1, 0.1);
    add(bgSky2);
    bgSky2.antialiasing = false;

    bgSchool2 = new BGSprite('stages/dead-pixel/weebSchool', repositionShit, 0, 0.6, 0.90);
    add(bgSchool2);
    bgSchool2.antialiasing = false;

    bgStreet2 = new BGSprite('stages/dead-pixel/weebStreet', repositionShit, 0, 0.95, 0.95);
    add(bgStreet2);
    bgStreet2.antialiasing = false;

    if(!ClientPrefs.lowQuality) {
        fgTrees2 = new BGSprite('stages/dead-pixel/weebTreesBack', repositionShit + 170, 130, 0.9, 0.9);
    ...
    bgTrees2.frames = Paths.getPackerAtlas('stages/dead-pixel/weebTrees');
    ...
    if(!ClientPrefs.lowQuality) {
        treeLeaves = new BGSprite('stages/dead-pixel/petals', repositionShit, -40, 0.85, 0.85, ['PETALS ALL'], true);
    ...
    bgTrees2.setGraphicSize(Std.int(widShit * 1.42));
